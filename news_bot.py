...
def build_message():
    ...
    for group_name, keywords in KEYWORD_GROUPS.items():
        ...
        group_articles.sort(key=lambda x: x["published_time"], reverse=True)

        message += f"■ {group_name}\n"

        for idx, article in enumerate(group_articles[:15], 1):
            message += f"{idx}. [{article['keyword']}] {article['title']}\n"
